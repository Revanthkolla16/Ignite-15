...
const ProductCard = ({ product, onClick, onAddToCart, cart, onQtyChange, onRemove }) => {
  ...
  return (
    <div
      className="bg-gray-800 rounded-2xl shadow-md p-6 cursor-pointer transition relative flex flex-col items-center group border border-gray-700 min-h-[240px] sm:min-h-[300px] justify-center hover:shadow-2xl hover:-translate-y-2 transition-transform transition-shadow duration-200"
      onClick={onClick}
    >
      {/* Image placeholder */}
      ...
      {!cartItem ? (
        <button
          className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-yellow-400 text-gray-900 px-5 py-2 rounded-full shadow hover:bg-yellow-500 text-sm font-semibold opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer"
          onClick={e => {
            e.stopPropagation();
            onAddToCart(product);
          }}
